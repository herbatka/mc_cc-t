...
- **Craft** — type a name, Enter to search a recipe database over HTTP (see
  ...
  It shows the exact ingredient list — what's needed and what you're short
  on — before you commit. If everything's in stock, Enter crafts it for
  real using a turtle, and the crafted item is delivered to OUTPUT (same
  chest withdrawals go to) so it's waiting for you to collect. Any leftover
